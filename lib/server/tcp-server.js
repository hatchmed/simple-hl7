...
function TcpServer(options, handler) {
  ...
  this.closeSocketAfterRespone = options.closeSocketAfterRespone !== false;
}
...
function Req(msg, raw) {
  ...
  // cna 20250204 added these fields
  this.msgTime = msg.header.getField(5);
  this.msgCtrlId = msg.header.getField(8);
}

function Res(socket, ack, closeSocketAfterRespone) {
  this.ack = ack;
  this.socket = socket;

  this.end = function() {
    var response = VT + (this.ack).toString() + FS + CR;

    // cna 20260901 close socket after response to avoid resource exhaustion(socket leaks)
    if (closeSocketAfterRespone) {
      socket.end(response);
      return;
    }
    ...
    socket.on('data', function(data) {
      try {
        ...
        if (message.substring(message.length - 2, message.length) == FS + CR) {
          ...
          var res = new Res(socket, ack, self.closeSocketAfterRespone);
